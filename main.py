...
def test_finale():
    ...
    #
    testFalliti += testEqual(inserisci(c, "Dexter", "Colin",
                             "Il mondo silenzioso di Nicholas Quinn", 2012, ("G", 15)), True)
    a = serializza(c)
    print(a)
    #
    testFalliti += testEqual(inserisci(c, "Adams", "Douglas", "Guida galattica per gli autostoppisti",
                             1980, ("F", 212), "Traduzione Laura Serra. "), True)
    a = serializza(c)
    print(a)
    #
    testFalliti += testEqual(inserisci(c, "Dexter", "Colin",
                             "L'ultima corsa per Woodstock", 2010, ("G", 14)), True)
    a = serializza(c)
    print(a)
    #
    testFalliti += testEqual(inserisci(c, "Dexter", "Colin",
                             "Niente vacanze per l'ispettore Morse", 2012, ("G", 16)), True)
    a = serializza(c)
    print(a)
    #
    testFalliti += testEqual(inserisci(c, "Simenon", "Georges",
                             "Gli intrusi", 2015, ("T", 33), "Donazione Giachi"), True)
    a = serializza(c)
    print(a)
    #
    ...
    inserisci(b, "Dexter", "Colin", "Le figlie di Caino",
              2017, ("G", 22), "Copia danneggiata. ")
    print(serializza(b))
    testFalliti += testEqual(sono_uguali(c, b), False)

    # test concatena
    print("""==========> Test concatena """)
    inserisci(c, "Dexter", "Colin", "Le figlie di Caino", 2017,
              ("G", 22), "In prestito biblioteca Lazzerini. ")
    print(serializza(c))
    d = concatena(c, b)
    print(serializza(d))
